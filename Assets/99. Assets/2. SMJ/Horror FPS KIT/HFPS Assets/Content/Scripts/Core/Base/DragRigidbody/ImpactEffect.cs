    ...
    public class ImpactEffect : MonoBehaviour
    {
        ...
        void OnImpact(float volume, Collision col)
        {
            if (useDefaultImpact && defaultImpact.impactSounds.Length > 0)
            {
                ...
                {
                    lastSound = randomUnique ? Utility.RandomUnique(0, defaultImpact.impactSounds.Length, lastSound) : Random.Range(0, defaultImpact.impactSounds.Length);
            ...
            else if (!useDefaultImpact)
            {
                ...
                foreach (var impact in impactSounds)
                {
                    if (impact.impactSounds.Length > 0)
                    {
                        ...
                        {
                            Texture2D terrainTex = Utility.TerrainPosToTex(terrain, transform.position);
